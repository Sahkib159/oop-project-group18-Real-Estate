...
public class assignTicketsController{
    ...
    void assignticketbutton(ActionEvent event) {
        ...
        try{
            ...
            AssignTickets y=new AssignTickets(email,pass,ticId,ticSummary,currentStatus);
            tableview.getItems().add(y);

            oos.writeObject(y);

        }catch(IOException ex){
            Logger.getLogger(assignTicketsController.class.getName()).log(Level.SEVERE, null, ex);
        }finally{
            try {
                if(oos!=null){
                    oos.close();
